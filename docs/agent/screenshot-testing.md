...
Для скриншотов требуется CDP-эндпоинт на запущенном приложении. По умолчанию попросите разработчика запустить приложение с удалённой отладкой WebView2. Если пользователь явно разрешил самостоятельный запуск, агент может запустить его сам, но обязан остановить только созданный им процесс после проверки:
