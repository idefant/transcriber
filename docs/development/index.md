...
Какие этапы обработки замеряются и где хранятся, почему клиентское время вызова не равно времени провайдера и что о нём сообщает каждый провайдер, зачем соединение прогревается в момент старта записи, почему аудио приводится к 16 кГц моно и почему стоимость генерации OpenRouter догружается в фоне, описано в [processing-timings.md](processing-timings.md).

## Разбор хвостов задержки

На какой выборке собранные метрики разбирались, как распределены задержки по этапам и маршрутам, почему тяжёлые выбросы отнесены к залипаниям на стороне провайдера, чем эта гипотеза не подтверждена, во что обошёлся бы повторный запрос и что мешает планировать исправление, описано в [latency-tail-analysis.md](latency-tail-analysis.md).
...
Почему пауза взведена только на время записи (а не всю сессию), почему она недоступна в режиме удержания, зачем событию `dictation-session` флаг `isRecording`, почему продолжение записи не должно очищать сэмплы, почему флаг паузы живёт внутри варианта `Recording` и почему пауза возвращает системный звук, описано в [pause-hotkey.md](pause-hotkey.md).

## Пауза системного медиа

Почему воспроизведение останавливается через SMTC, а не медиа-клавишей, чего этот режим принципиально не покрывает, почему пауза (в отличие от заглушения) ставится до старта захвата и с ожиданием, почему у ожидания нет своего состояния оверлея, зачем прогревается менеджер сессий и почему возобновляются только сессии, остановленные самим приложением, описано в [media-pause.md](media-pause.md).
...
Почему React Compiler сознательно не подключён и мемоизация ручная, правила атомарных селекторов Zustand и мемоизации значения контекста, почему `memo` окупается только со стабильными пропсами (разбор `HistoryRecordsList`/`HistoryRecordRow` и связанного инварианта `useCallback` в `HistoryPage`) и что сознательно не мемоизировано (оверлеи, аудиоплеер), описаны в [memoization.md](memoization.md).
...
Почему поток захвата собирается один раз при запуске и переиспользуется (в приостановленном состоянии) между сессиями, как это убирает задержку открытия WASAPI-клиента из горячего пути старта диктовки, перенос mute/чтения настроек с этого пути, пересборка при смене устройства по умолчанию, почему индикатор микрофона остаётся достоверным, и порядок блокировки mutex описаны в [recording-prewarm.md](recording-prewarm.md).

## Речевая категория потока захвата

Почему микрофон открывается с `AudioCategory_Speech`, как обработка производителя иначе отдавала битовые нули, почему из-за этого пришлось уйти от `cpal` на собственный модуль поверх крейта `windows`, почему формат читается уже после установки категории и что делает откат для устройств, которые речевой поток не принимают, описано в [speech-audio-category.md](speech-audio-category.md).
...
Почему оверлей записи занимает два окна на монитор, почему тень не может лежать в окне карточки, почему нативная тень окна была реализована и отклонена, и какие инварианты держат карточку и её тень синхронными, описано в [overlay-shadow.md](overlay-shadow.md).
