...
Почему пауза взведена только на время записи (а не всю сессию), почему она недоступна в режиме удержания, зачем событию `dictation-session` флаг `isRecording`, почему продолжение записи не должно очищать сэмплы и почему флаг паузы живёт внутри варианта `Recording`, описано в [pause-hotkey.md](pause-hotkey.md).
