...
Внутренние ошибки VAD дополнительно записываются как `vad.failed` даже при выключенном расширенном отладочном логировании. Классификация позволяет различить сбой ресемплинга, недоступную ONNX Runtime, ошибку загрузки модели и ошибку инференса. Обычные результаты `noSpeech` и `speechTooShort` не являются внутренними сбоями и записываются только при включённом расширенном отладочном логировании.
