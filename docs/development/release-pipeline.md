...
Релизы полностью автоматизированы через GitHub Actions. Триггером служит push git-тега, соответствующего `v*` (например, `v1.2.3` или `v1.2.3-beta.1`). Кроме простановки тега, никакого ручного вмешательства не требуется. При ручном запуске workflow нужно указать такой же тег: сборка берёт исходный код именно этого тега, а не ветки, из которой был открыт запуск.
